...
<body class="bg-gray-100 flex items-center justify-center min-h-screen">
    <div class="text-center">
        ...
        <div class="mb-4">
            <p class="text-lg"><?php
             $query = "SELECT * FROM tbl_javascript where id = 8";
             ...
             $row = mysqli_num_rows($result);
              if ($row > 0) {
               ...
               <br></p>
            <input type="radio" name="option" value="let" id="let">
            <label for="let" class="ml-2"><?php
             $query = "SELECT * FROM tbl_javascript where id = 8";
             $result = mysqli_query($con,$query);
             $row = mysqli_num_rows($result);
              if ($row > 0) {
            ...
            <input type="radio" name="option" value=" var" id=" var">
            <label for=" var" class="ml-2"><?php
             $query = "SELECT * FROM tbl_javascript where id = 8";
             $result = mysqli_query($con,$query);
             $row = mysqli_num_rows($result);
              if ($row > 0) {
                while ($output = mysqli_fetch_assoc($result)) { ?>
                    <?php echo $output['B']; ?> 
               <?php }} else {echo 'No questions found'."<br>";} ?></label><br>
            <input type="radio" name="option" value=" final" id=" final">
            <label for=" final" class="ml-2"><?php
             $query = "SELECT * FROM tbl_javascript where id = 8";
             $result = mysqli_query($con,$query);
             $row = mysqli_num_rows($result);
              if ($row > 0) {
                while ($output = mysqli_fetch_assoc($result)) { ?>
                    <?php echo $output['C']; ?> 
               <?php }} else {echo 'No questions found'."<br>";} ?></label><br>
            <input type="radio" name="option" value="const" id="const">
            <label for="const" class="ml-2"><?php
             $query = "SELECT * FROM tbl_javascript where id = 8";
             $result = mysqli_query($con,$query);
             $row = mysqli_num_rows($result);
              if ($row > 0) {
