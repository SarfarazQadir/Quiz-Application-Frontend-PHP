...
<body class="bg-gray-100 flex items-center justify-center min-h-screen">
    <div class="text-center">
        ...
        <div class="mb-4">
            <p class="text-lg"><?php
             $query = "SELECT * FROM tbl_javascript where id = 3";
             ...
             $row = mysqli_num_rows($result);
              if ($row > 0) {
               ...
               <br></p>
            <input type="radio" name="option" value="parseInteger()" id="parseInteger()">
            <label for="parseInteger()" class="ml-2"><?php
             $query = "SELECT * FROM tbl_javascript where id = 3";
             $result = mysqli_query($con,$query);
             $row = mysqli_num_rows($result);
              if ($row > 0) {
            ...
            <input type="radio" name="option" value=" toInteger()" id=" toInteger()">
            <label for=" toInteger()" class="ml-2"><?php
             $query = "SELECT * FROM tbl_javascript where id = 3";
             $result = mysqli_query($con,$query);
             $row = mysqli_num_rows($result);
              if ($row > 0) {
                while ($output = mysqli_fetch_assoc($result)) { ?>
                    <?php echo $output['B']; ?> 
               <?php }} else {echo 'No questions found'."<br>";} ?></label><br>
            <input type="radio" name="option" value="parseInt()" id="parseInt()">
            <label for="parseInt()" class="ml-2"><?php
             $query = "SELECT * FROM tbl_javascript where id = 3";
             $result = mysqli_query($con,$query);
             $row = mysqli_num_rows($result);
              if ($row > 0) {
                while ($output = mysqli_fetch_assoc($result)) { ?>
                    <?php echo $output['C']; ?> 
               <?php }} else {echo 'No questions found'."<br>";} ?></label><br>
            <input type="radio" name="option" value=" parseFloat()" id=" parseFloat()">
            <label for=" parseFloat()" class="ml-2"><?php
             $query = "SELECT * FROM tbl_javascript where id = 3";
             $result = mysqli_query($con,$query);
             $row = mysqli_num_rows($result);
              if ($row > 0) {
