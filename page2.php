...
<body class="bg-gray-100 flex items-center justify-center min-h-screen">
    <div class="text-center">
        ...
        <div class="mb-4">
            <p class="text-lg"><?php
             $query = "SELECT * FROM tbl_javascript where id = 2";
             ...
             $row = mysqli_num_rows($result);
              if ($row > 0) {
                while ($output = mysqli_fetch_assoc($result)) { ?>
                    <?php echo $output['B']; ?> 
               <?php }} else {echo 'No questions found'."<br>";} ?></label><br>
            <input type="radio" name="option" value="null" id="null">
            <label for="null" class="ml-2"><?php
             $query = "SELECT * FROM tbl_javascript where id = 2";
             $result = mysqli_query($con,$query);
             $row = mysqli_num_rows($result);
              if ($row > 0) {
                while ($output = mysqli_fetch_assoc($result)) { ?>
                    <?php echo $output['C']; ?> 
               <?php }} else {echo 'No questions found'."<br>";} ?></label><br>
            <input type="radio" name="option" value="object" id="object">
            <label for="object" class="ml-2"><?php
             $query = "SELECT * FROM tbl_javascript where id = 2";
             $result = mysqli_query($con,$query);
             $row = mysqli_num_rows($result);
              if ($row > 0) {
