...
<body class="bg-gray-100 flex items-center justify-center min-h-screen">
    <div class="text-center">
        ...
        <div class="mb-4">
            <p class="text-lg"><?php
             $query = "SELECT * FROM tbl_javascript where id = 6";
             ...
             $row = mysqli_num_rows($result);
              if ($row > 0) {
               ...
               <br></p>
            <input type="radio" name="option" value="push()" id="push()">
            <label for="push()" class="ml-2"><?php
             $query = "SELECT * FROM tbl_javascript where id = 6";
             $result = mysqli_query($con,$query);
             $row = mysqli_num_rows($result);
              if ($row > 0) {
            ...
            <input type="radio" name="option" value="append()" id="append()">
            <label for="append()" class="ml-2"><?php
             $query = "SELECT * FROM tbl_javascript where id = 6";
             $result = mysqli_query($con,$query);
             $row = mysqli_num_rows($result);
              if ($row > 0) {
             ...
             $row = mysqli_num_rows($result);
              if ($row > 0) {
                while ($output = mysqli_fetch_assoc($result)) { ?>
                    <?php echo $output['C']; ?> 
               <?php }} else {echo 'No questions found'."<br>";} ?></label><br>
            <input type="radio" name="option" value="insert()" id="insert()">
            <label for="insert()" class="ml-2"><?php
             $query = "SELECT * FROM tbl_javascript where id = 6";
             $result = mysqli_query($con,$query);
             $row = mysqli_num_rows($result);
              if ($row > 0) {
