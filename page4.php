...
<body class="bg-gray-100 flex items-center justify-center min-h-screen">
    <div class="text-center">
        ...
        <div class="mb-4">
            <p class="text-lg"><?php
             $query = "SELECT * FROM tbl_javascript where id = 4";
             ...
             $row = mysqli_num_rows($result);
              if ($row > 0) {
               ...
               <br></p>
            <input type="radio" name="option" value="NaN" id="NaN">
            ...
            <input type="radio" name="option" value="Error" id="Error">
            <label for="Error" class="ml-2"><?php
             $query = "SELECT * FROM tbl_javascript where id = 4";
             $result = mysqli_query($con,$query);
             $row = mysqli_num_rows($result);
              if ($row > 0) {
            ...
            <label for="false" class="ml-2"><?php
             $query = "SELECT * FROM tbl_javascript where id = 4";
             $result = mysqli_query($con,$query);
             $row = mysqli_num_rows($result);
              if ($row > 0) {
